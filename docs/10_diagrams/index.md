...
#### C4 Component Diagram (`c4_component.puml`)

Диаграмма компонентов описывает внутреннюю структуру ключевого контейнера UC-платформы — **Call Control Service** — на уровне компонентов и их взаимодействий:
...
- **SIP Handler** и **WebRTC Handler** — обработка протокольной сигнализации
- **CDR Collector** — асинхронный сбор данных для CDR

Диаграмма демонстрирует принципы модульной архитектуры и разделения ответственности: оркестрация отделена от логики маршрутизации, управление сессиями изолировано от обработки протоколов, сбор CDR выполняется асинхронно. Диаграмма в `c4_component.puml` поддерживает драйверы **AD-01**, **AD-02**, **AD-03** и решения из ADR-001, ADR-002, ADR-005.
...
#### Database Diagram (`database_diagram.puml`)

Диаграмма баз данных описывает логическую структуру хранилищ данных UC-платформы на уровне таблиц, их полей и связей:

- **DB_ACCOUNTS** (PostgreSQL) — аккаунты, профили, устройства, feature-флаги, правила маршрутизации
- **DB_CALLS** (PostgreSQL) — сессии вызовов, ноги вызовов, маршрутизация, очереди
- **DB_CDR** (PostgreSQL) — детализация вызовов, тарифные планы, биллинг аккаунтов
- **DB_MESSAGING** (PostgreSQL) — беседы, сообщения, presence статусы и подписки
...
Диаграмма демонстрирует разделение данных по доменам, выбор типов хранилищ (реляционные и нереляционные) и логические связи между доменами на уровне приложения. Диаграмма в `database_diagram.puml` поддерживает драйверы **AD-01**, **AD-03**, **AD-05** и решения из ADR-003, ADR-010.

#### NoSQL Database Diagram (`nosql_database.puml`)

Диаграмма нереляционных хранилищ описывает структуру и использование специализированных хранилищ данных UC-платформы:

- **Object Storage (S3-совместимое)** — записи разговоров и медиа-файлы (recording_bucket, media_assets_bucket)
- **Redis (Key-Value)** — кэш сессий, токенов, presence, активных чатов, rate limiting
- **Time-Series Database** — метрики системы и качества вызовов
- **Log Storage** — логи приложений и аудит безопасности
- **Trace Storage** — распределённая трассировка для анализа производительности

Диаграмма демонстрирует специализацию хранилищ под различные паттерны доступа и оптимизацию производительности через кэширование. Диаграмма в `nosql_database.puml` поддерживает драйверы **AD-01**, **AD-03**, **AD-05** и решения из ADR-003, ADR-005, ADR-010.
